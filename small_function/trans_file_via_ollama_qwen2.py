...
def make_tran_qwen2(srt_path, out_path=None, opt="字幕", num_of_line=5):
    """clean代表是否对结果进行规则处理[此参数已废弃]"""
    if out_path and os.path.exists(out_path):
        os.remove(out_path)

    if "字幕" in opt:
        # todo: 可以选定从指定的srt文件的第i个字幕开始翻译
            ...
            for j, row in enumerate(li):
                ...
                if row == str(i * num_of_line + 1) + "\n" or j == len(li) - 1:
                    ...
                    if result is None:
                        ...
                        print(f"未处理-\n{result}")
                        result = resp_handler(result, num_of_line)
...
def resp_handler_with_id(s: str, num: int, start_id: int):
    ...
    for j in range(num):
        ...
        ind2 = s[ci1 + 10:].find(":")  # 标示第2个时间戳的位置
        ...
        if j == num - 1:
            ...
            next_ind, _ = get_line_id(s, next_id)
...
def main():
    ...
    make_tran_qwen2(inp_path, out_path, opt, nol)
